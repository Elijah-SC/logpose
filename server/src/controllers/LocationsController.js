...
export class LocationsController extends BaseController {
  constructor() {
    ...
    this.router
      ...
      .get("/:locationId/comments", this.getLocationComments)
  ...
  async getLocationComments(request, response, next) {
    try {
      const locationId = request.params.locationId;
      const locationComments = await commentsService.getLocationComments(
        locationId
      );
      response.send(locationComments);
    } catch (e) {
      next(e);
    }
  }
